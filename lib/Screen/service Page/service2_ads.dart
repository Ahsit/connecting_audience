// ignore_for_file: prefer_const_constructors

import 'package:flutter/material.dart';

import 'package:ricoz_app/pages/menu_page.dart';

class ServiceGoogleAds extends StatelessWidget {
  const ServiceGoogleAds({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    double width = MediaQuery.of(context).size.width;
    return Scaffold(
        backgroundColor: const Color.fromRGBO(68, 1, 1, 1),
        appBar: AppBar(
            backgroundColor: const Color.fromRGBO(68, 1, 1, 1),
            automaticallyImplyLeading: false,
            title: const Image(
                image: AssetImage('assets/image 1 (4).png'),
                width: 110,
                height: 50),
            centerTitle: true,
            leading: GestureDetector(
              onTap: () {
                Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => const MenuPage(),
                    ));
              },
              child: const Image(
                image: AssetImage('assets/image 125 (4).png'),
              ),
            ),
            actions: const [
              Padding(
                  padding: EdgeInsets.only(right: 10),
                  child: Icon(
                    Icons.notifications,
                    size: 30,
                  )),
            ]),
        body: SingleChildScrollView(
          child: Column(
            children: [
              const Padding(
                padding: EdgeInsets.all(20.0),
                child: Image(image: AssetImage('assets/adwords.png')),
              ),
              Container(
                  height: 540,
                  width: width * 0.9,
                  decoration: const BoxDecoration(
                    color: Colors.white,
                    borderRadius: BorderRadius.all(Radius.circular(8)),
                  ),
                  child: Column(
                    children: [
                      const SizedBox(height: 14),
                      const Text('Google Ads',
                          style: TextStyle(
                              color: Colors.black,
                              fontSize: 21,
                              fontWeight: FontWeight.w500)),
                      const SizedBox(height: 14),
                      const Image(
                          image: AssetImage('assets/image 189.png'),
                          width: 100,
                          height: 100),
                      const SizedBox(height: 20),
                      Padding(
                        padding:
                            EdgeInsets.only(left: 23.0, right: 23, top: 20),
                        child: Text(
                            'Custom Intent Audiences + Remarketing List for Search Ads (RLSA)',
                            style: TextStyle(
                                fontSize: 17, fontWeight: FontWeight.w400)),
                      ),
                      Padding(
                        padding:
                            EdgeInsets.only(left: 23.0, right: 23, top: 20),
                        child: Text(
                            'Custom Intent Audiences + Facebook Video Ads',
                            style: TextStyle(
                                fontSize: 17, fontWeight: FontWeight.w400)),
                      ),
                      Padding(
                        padding:
                            EdgeInsets.only(left: 23.0, right: 23, top: 20),
                        child: Text(
                            'Custom Intent Audiences + YouTube TrueView for Action',
                            style: TextStyle(
                                fontSize: 17, fontWeight: FontWeight.w400)),
                      ),
                      Padding(
                        padding:
                            EdgeInsets.only(left: 23.0, right: 23, top: 20),
                        child: Text(
                            'Custom Intent Audiences + LinkedIn Sponsored InMail',
                            style: TextStyle(
                                fontSize: 17, fontWeight: FontWeight.w400)),
                      ),
                      SizedBox(
                        height: 50,
                      ),
                      Container(
                        width: 277,
                        height: 47,
                        decoration: const BoxDecoration(
                          color: Color.fromRGBO(68, 1, 1, 1),
                          borderRadius: BorderRadius.all(
                            Radius.circular(42),
                          ),
                        ),
                        child: const Center(
                            child: Text('Start your learning',
                                style: TextStyle(
                                    fontSize: 20,
                                    fontWeight: FontWeight.w600,
                                    color: Colors.white))),
                      ),
                      const SizedBox(height: 20),
                    ],
                  )),
              const SizedBox(height: 26),
              const Divider(
                color: Colors.white,
                thickness: 5,
                indent: 138,
                endIndent: 138,
              )
            ],
          ),
        ));
  }
}
